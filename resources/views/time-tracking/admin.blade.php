...
@section('title', 'Admin Time Tracking')
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="py-6">
        ...
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Time Tracking Overview</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Monitor guest time tracking across the workspace</p>
            </div>
                ...
                <div class="border-t border-gray-200 dark:border-gray-700 pt-4">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <a href="{{ route('time-tracking.index', array_merge(request()->except('track_id'))) }}" 
                           class="px-4 py-2 rounded-md text-sm font-medium whitespace-nowrap {{ !($trackFilter ?? null) ? 'bg-indigo-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600' }}">
                            All Tracks
                        </a>
                        @foreach($tracks ?? [] as $track)
                            <a href="{{ route('time-tracking.index', array_merge(request()->except('track_id'), ['track_id' => $track->id])) }}" 
                               class="px-4 py-2 rounded-md text-sm font-medium flex items-center whitespace-nowrap {{ ($trackFilter ?? null) == $track->id ? 'bg-indigo-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600' }}">
                                <div class="h-3 w-3 rounded-full mr-2 flex-shrink-0" style="background-color: {{ $track->color }}"></div>
                                {{ $track->name }}
                            </a>
                        @endforeach
                    </div>
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                ...
                                <dd class="text-2xl font-bold text-gray-900 dark:text-white">{{ $totalFormatted }}</dd>
                            </dl>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow rounded-lg">
                <div class="p-5">
                    <div class="flex items-center">
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 truncate">Active Guests</dt>
                                <dd class="text-2xl font-bold text-gray-900 dark:text-white">{{ $guestTimeData->where('entries_count', '>', 0)->count() }}</dd>
                            </dl>
                        </div>
                    </div>
                </div>
        ...
        <!-- Guest Time Tracking Table -->
        <div class="bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden">
            <div class="px-4 py-5 sm:p-6">
                <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Guest Time Tracking</h3>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse($guestTimeData as $data)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700" x-data="{ showDetails: false }">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        @else
                                            <span class="text-sm text-gray-400 dark:text-gray-500">No track</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">
                                            {{ $data['entries_count'] }} {{ $data['entries_count'] === 1 ? 'entry' : 'entries' }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        ...
                                        @else
                                            <span class="text-gray-400 dark:text-gray-500">No entries</span>
                                        @endif
                                    </td>
                                </tr>
                                @if($data['entries_count'] > 0)
                                    <tr x-show="showDetails" x-cloak>
                                        <td colspan="5" class="px-6 py-4 bg-gray-50 dark:bg-gray-700">
                                            <div class="space-y-2">
                                                <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-3">Time Entries</h4>
                                                @foreach($data['entries'] as $entry)
                                                    <div class="flex items-center justify-between p-3 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-600">
                                                        <div class="flex-1">
                                                            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $entry->task?->title ?? 'No task' }}</p>
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center">
                                        ...
                                        <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">No guests found</h3>
                                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">There are no guests with time entries in this workspace.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
