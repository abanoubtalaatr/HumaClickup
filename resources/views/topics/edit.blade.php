...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="py-6">
        ...
        <div class="mb-6">
            ...
            <p class="mt-1 text-sm text-gray-500">Update topic information</p>
        </div>
            ...
            <form action="{{ route('topics.update', $topic) }}" method="POST" class="p-6">
                ...
                <div class="mb-4">
                    ...
                    <textarea id="description" 
                              name="description" 
                              rows="4"
                              class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('description', $topic->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="url" 
                           ...
                           value="{{ old('presentation_link', $topic->presentation_link) }}"
                           placeholder="https://..."
                           class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                ...
                <div class="mb-4">
                    ...
                    <input type="date" 
                           id="date" 
                           name="date" 
                           value="{{ old('date', $topic->date->format('Y-m-d')) }}"
                           required
                           class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                    @error('date')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="mb-4">
                    ...
                    <select id="track_id" 
                        ...
                        @foreach($tracks as $track)
                            <option value="{{ $track->id }}" {{ old('track_id', $topic->track_id) == $track->id ? 'selected' : '' }}>
                                {{ $track->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    <div class="flex items-center">
                        <input type="checkbox" 
                               ...
                               name="is_complete" 
                               value="1"
                               {{ old('is_complete', $topic->is_complete) ? 'checked' : '' }}
