        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($projects ?? [] as $project)
                <div class="bg-white rounded-lg shadow hover:shadow-lg transition-shadow overflow-hidden flex flex-col">
                    <a href="{{ route('projects.show', $project) }}" class="block p-6 flex-1 flex flex-col">
                        <!-- Header -->
                        <div class="flex items-start justify-between mb-4">
                            <div class="flex items-center flex-1 min-w-0">
                                ...
                                <div class="ml-4 flex-1 min-w-0">
                                    ...
                                    @if($project->space)
                                        <p class="text-sm text-gray-500 truncate">{{ $project->space->name }}</p>
                                    @endif
                                </div>
                            </div>
                        </div>
                        ...
                        @if(!$isGuest && $project->createdBy)
                            <div class="mb-4 pb-4 border-b border-gray-200">
                                <div class="flex items-center space-x-2 mb-2">
                                    ...
                                    <span class="text-xs font-medium text-gray-900">{{ ucwords(strtolower($project->createdBy->name)) }}</span>
                                </div>
                                @if($project->createdBy->whatsapp_number)
                                    <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $project->createdBy->whatsapp_number) }}" 
                                       target="_blank"
                                       onclick="event.stopPropagation(); event.preventDefault();"
                                       class="inline-flex items-center px-2 py-1 bg-green-100 text-green-700 rounded hover:bg-green-200 transition-colors text-xs w-fit">
                                        <svg class="h-3 w-3 mr-1 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <span class="truncate">{{ $project->createdBy->whatsapp_number }}</span>
                                    </a>
                                @endif
                            </div>
                        @endif
