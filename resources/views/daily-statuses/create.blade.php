...
@section('title', 'Add Daily Status')
...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="py-6">
        <!-- Header -->
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900">Add Daily Status</h1>
            <p class="mt-1 text-sm text-gray-500">
                @php
                    $statusDate = \Carbon\Carbon::parse($date);
                @endphp
                Status for {{ $statusDate->format('l, F d, Y') }}
            </p>
        </div>

        <!-- Form -->
        <div class="bg-white shadow rounded-lg">
            <form action="{{ route('daily-statuses.store') }}" method="POST" class="p-6">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <textarea name="status" 
                              id="status" 
                              style="display: none;">{{ old('status') }}</textarea>
